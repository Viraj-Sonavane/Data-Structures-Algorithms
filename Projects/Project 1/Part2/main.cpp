...
int partition(const string &s,vector<int> &v, int low, int high, int pivotindex);

void readFromFile(string &s, string file);

void convertToLower(string &s);

void insertion(const string &s,vector<int> &v, int low, int high);
...
int main(int argc, char* argv[])
{
    ...
    if(argc==3)
    {
       ...
       k = stoi(argv[2]);
       k = select(s,indices,0,indices.size()-1,k);
       cout<<k;
       cout<<endl;
    }
